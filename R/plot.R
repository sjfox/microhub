...
prepare_historic_data <- function(data, cleaned_forecasts_quantiles, start_date="2024-01-01") {
  ...
  forecast_df <- cleaned_forecasts_quantiles %>%
    ...
    mutate(target_end_date = as.Date(target_end_date),  # Ensure dates are formatted
  ...
  historic_season_truth <- merge(data, historic_season_data, by = c("year", "epiweek", "age_group"), all = FALSE)
  historic_forecast_truth <- merge(data, historic_forecast_data, by = c("year", "epiweek", "age_group"), all = FALSE)
...
############################
plot_state_forecast_try <- function(forecast_date,
                                    curr_location_name,
                                    curr_season_data, 
                                    forecast_df,
                                    historic_data) {  # Add historic_data as a parameter
  ...
  ggplot() +
    geom_ribbon(data = forecast_df, aes(epiweek, ymin = `0.025`, ymax = `0.975`), alpha = .2) +
    geom_ribbon(data = forecast_df, aes(epiweek, ymin = `0.25`, ymax = `0.75`), alpha = .2) +
    geom_line(data = forecast_df, aes(epiweek, `0.5`)) +
    geom_point(data = curr_df, aes(epiweek, count)) +
    geom_point(data = historic_df, aes(epiweek, count), color = "red") +  # Add the historic data points in red
    geom_point(data = last_point, aes(epiweek, count), color = "blue", size = 2) +
