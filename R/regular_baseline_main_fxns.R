# =====================================
# Step 1: Wrangle Baseline Data
# =====================================
wrangle_baseline <- function(dataframe, forecast_date, data_to_drop) {
  library(dplyr)
  library(readr)
  library(lubridate)
  library(epiprocess)
  ...
  target_tbl <- read_csv(
    ...
    filter(target_group %in% c("Pediatric", "Adult", "Overall")) %>%
...
# =====================================
get_quantiles_df <- function(sim_matrix, taus = c(0.01, 0.025, seq(0.05, 0.95, 0.05), 0.975, 0.99)) {
...
# =====================================
# Step 2: Fit + Process Baseline Forecasts (All Data)
# =====================================
fit_process_baseline <- function(target_edf, forecast_date, desired_max_time_value, base_weeks_ahead, forecast_horizons, output_dir) {
  library(dplyr)
  library(tidyr)
  library(readr)
  library(simplets)
  ...
  preds_formatted <- preds %>%
    ...
    select(
      reference_date, horizon, target_end_date, target_group,
  ...
  output_file <- file.path(output_dir, paste0(format(forecast_date), "-regular-baseline.csv"))
  write_csv(preds_formatted, output_file)

  cat("✅ Regular baseline forecast saved with", nrow(preds_formatted), "rows at", output_file, "\n")
