fit_process_copycat <- function(fit_df,
                                historic_df, 
                                forecast_date, 
                                data_to_drop,
  ...
  for (curr_age_group in age_groups) {
    fit_df |>
      ...
      paraguay_copycat(db = traj_db, recent_weeks_touse = 100, resp_week_range = 1, forecast_horizon = 4 + weeks_to_drop) |>
...
paraguay_copycat <- function(curr_data,
                             forecast_horizon = 5, ## How many weeks forecast and plotted?
                             recent_weeks_touse = 5, ## 100 means all data from season are used
                             nsamps = 1000,
                             resp_week_range = 0,
  ...
  trajectories |> 
    ...
    ungroup() |> 
    mutate(forecast = most_recent_value*mult_factor) |> ##Want more dispersion than poisson distribution
